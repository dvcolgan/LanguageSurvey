...
class DiceReporter(object):
    self.dice
    def find_n_of_a_kind(n)
    def remaining_to_str
    def set_aside_to_str
    def 

class DiceEvaluator(object):
    def dice_combination_value

class Turn(object):
    self.remaining as Dice
    self.set_aside as Dice

class Dice(object):
    @classmethod
    def roll(count):
        dice = Dice()
        dice.values = [random.randint(1,6) for die in self.remaining]
        return dice

    @classmethod
    def set_as(*values):
        dice = Dice()
        dice.values = [value for value in *values]
        return dice



    def __init__(self, count=0, initial_values=None, roll=False):
        if initial_values:
...
class Dice(object):
    ...
    def count_dice(self, dice_values):
    ...
    def dice_combination_value(self, dice_values):
        score = 0
        die_counts = self.count_dice(dice_values)
        ...
        # strait
        if len(set(range(1,7)) - set(self.remaining)) == 0:
            score += 1500
            for i in range(1,7):
                die_counts[i] = 0
...
#abstract out the setting of the die_counts to 0 ?
    ...
    def check_for_farkle(self):
        if self.remaining == [0,0,0,0,0,0]: return
        value, leftovers = self.dice_combination_value(self.remaining)
        if value == 0:
            self.score = 0
